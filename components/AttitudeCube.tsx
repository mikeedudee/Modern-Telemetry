...
interface AttitudeCubeProps {
  ...
  gy: number; // Roll
  gz: number; // Heading
  vehicleIcon?: string; // AETHER: Added to control pitch offset
  showShadows?: boolean;
...
// AETHER NOTE: Using 0.1 for smooth interpolation (matching v10 stability)
...
export const AttitudeCube: React.FC<AttitudeCubeProps> = ({ 
  ...
  // AETHER FIX: Conditional Pitch Logic based on Vehicle Icon
  useEffect(() => {
      let pitchRad: number;

      // Logic:
      // Rocket: Sensor 90° (Vertical) -> Scene 0° (Upright). Correction: -90.
      // Plane/Drone: Sensor 0° (Level) -> Scene 0° (Level). Correction: 0.
      if (vehicleIcon === 'rocket') {
          pitchRad = THREE.MathUtils.degToRad(gx - 90);
      } else {
          pitchRad = THREE.MathUtils.degToRad(gx);
      }

      targetRot.current = {
          x: pitchRad,
          y: THREE.MathUtils.degToRad(-gz), // Yaw (Inverted for correct visual turn)
          z: THREE.MathUtils.degToRad(-gy)  // Roll (Inverted for correct visual tilt)
      };
  }, [gx, gy, gz, vehicleIcon]);
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (pivotRef.current) {
          ...
          // AETHER FIX: Restored v10 Stability Order 'ZYX'
