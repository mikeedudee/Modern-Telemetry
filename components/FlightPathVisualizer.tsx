...
import { 
  Lock, Unlock, Info, Crosshair, Target, Navigation, 
...
export const FlightPathVisualizer: React.FC<FlightPathProps> = ({ 
  ...
  const toLocal = (lat: number, lon: number, alt: number) => {
      ...
      const z = -(lat - originRef.current.lat) * metersPerDegLat; 
      const x = (lon - originRef.current.lon) * metersPerDegLon; 
      const y = alt; 
  ...
  useEffect(() => {
      ...
      if (vehicleRef.current && Math.abs(lastPacket.latitude) > 0.0001) {
          ...
          // AETHER MODIFICATION: Conditional Pitch Correction
          // If Rocket: Counter 90deg launch pitch (Sensor 90 -> Scene 0/Up)
          // If Plane/Drone: Raw pitch (Sensor 0 -> Scene 0/Horizon)
          let pitchRad: number;
          if (settings.graphics.vehicleIcon === 'rocket') {
              pitchRad = THREE.MathUtils.degToRad(pitchDeg - 90);
          } else {
  ...
  }, [history, prediction, isLocked, settings.graphics.vehicleIcon]); // Added vehicleIcon to dependency
